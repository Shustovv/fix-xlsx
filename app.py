...
async def normalize_xlsx(file: UploadFile = File(...)):
    ...
    if not soffice:
        return JSONResponse(
            status_code=500,
            content={"error": "LibreOffice binary not found"},
        )
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # LibreOffice обычно пишет output с тем же именем (input.xlsx)
        candidates = [
    ...
    # ВАЖНО: без Content-Disposition, чтобы не падать на кириллице в filename
    return Response(
        content=content,
